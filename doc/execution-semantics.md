...
An agent-owned `in_progress` issue is stalled when it has no active run, no queued continuation, no persisted monitor, no armed task watchdog with an invokable agent, and no explicit recovery surface. An unmanaged local/background watcher does not satisfy any of those conditions. A Paperclip-tracked run that is still running but silent is not automatically stalled; it is handled by the active-run watchdog contract.
